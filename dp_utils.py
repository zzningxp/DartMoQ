...
def plot_block_losses_overlap(
    ...
    # First collect all loss values from all quant types to find global min/max
    all_loss_values_list = []
    for data in all_block_data:
        if data is not None:
            _, bit_to_idx, all_block_losses = data
            for block_losses in all_block_losses:
                all_loss_values_list.append(block_losses.flatten())

    global_min = None
    global_max = None
    if all_loss_values_list:
        all_loss_values = np.concatenate(all_loss_values_list)
        global_min = np.min(all_loss_values)
        global_max = np.max(all_loss_values)
        # Add some padding
        global_min *= 0.9
        global_max *= 1.1
    ...
    for ax_idx, (quant_type, data) in enumerate(zip(quant_types, all_block_data)):
        ...
        expert_spacing = 0.8 / max(n_experts, 1)  # Spread experts within 0.8 range around bit
