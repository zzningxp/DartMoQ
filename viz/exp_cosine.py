...
python viz/exp_cosine.py \
    --model "$modelname" \
    --dataset wikitext2 \
    --nsamples 64 \
    --sensitivity-source inline \
    --sensitivity-seed 42 \
    --layers 17 \
    --experts 2 \
    --out-dir viz \
    --task-name cosine_deepseekv1 \
    --seeds 0 42 84 126 168 \
    --preserve-fracs 0.00 0.01 0.02 0.05
...
from viz._cache_io import model_label, resolve_model_id, resolve_model_path  # type: ignore
from viz.seed_stability import Target, _get_expert_weights, _load_expert_sensitivity, select_targets  # type: ignore


def _resolve_device(device: str) -> torch.device:
    if device.startswith("cuda") and not torch.cuda.is_available():
        print(f"[exp_cosine] requested {device}, but CUDA is unavailable; using CPU")
...
def _resolve_targets_with_sensitivity(
    ...
    if args.sensitivity_source == "cache":
        ...
        sensitivity_map = {
            (target.layer_idx, target.expert_idx): _load_expert_sensitivity(
                model_id, target.layer_idx, target.expert_idx, args.bit, args.quantmode, args.rank_mode
            )
            for target in targets
        }
        return targets, sensitivity_map, {}

    if not args.layers:
        raise ValueError("--layers is required when --sensitivity-source inline")
    ...
    for layer_idx in args.layers:
        layer_inputs = _collect_layer_mlp_inputs(model, layer_idx, dataloader, args.nsamples, device)
        ...
        expert_scores = [
            (expert_idx, float(torch.as_tensor(rates).sum().item()))
            for expert_idx, rates in enumerate(all_rates)
        ]
        if args.experts:
            expert_set = set(args.experts)
            expert_scores = [(e, s) for e, s in expert_scores if e in expert_set]
        else:
            expert_scores.sort(key=lambda item: item[1], reverse=True)
            expert_scores = expert_scores[:args.top_experts]

        for expert_idx, score in expert_scores:
            target = Target(layer_idx=layer_idx, expert_idx=expert_idx, total_sensitivity=score)
...
def _cosine_out_root(args) -> Path:
    # 默认直接存到项目 viz/ 下，目录名使用“任务名_时间”，方便按任务查找结果。
    task_name = args.task_name or os.environ.get("SLURM_JOB_NAME") or "cosine_similarity"
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    return Path(args.out_dir) / f"{_slug(task_name)}_{timestamp}"


def run_cosine_experiment(args) -> None:
    ...
    # 加载校准数据，用于 inline sensitivity 计算和收集 expert 输入 token。
    ...
    for target in targets:
        ...
        # 收集被当前 expert 路由到的真实 token 输入，保证局部输出比较更贴近实际激活分布。
        ...
        # 原始 fp16 expert 输出作为参考输出 y_ref。
        ref_out = _expert_forward_with_weights(expert, tokens, up_w, gate_w, down_w)

        per_frac = []
        for frac in preserve_fracs:
            # 取 sensitivity 最高的 frac 比例神经元，这些 neuron 后续保持 fp16。
            keep_idx = _mask_indices_by_frac(sensitivity, frac, largest=True)
            seed_metrics = []
            for seed in args.seeds:
                # 对同一个 expert 使用不同 TurboQuant seed 做 2bit/低比特量化。
                up_q = _turboquant_weight(up_w, args.bit, args.group_size, seed, args.rotation)
                gate_q = _turboquant_weight(gate_w, args.bit, args.group_size, seed, args.rotation)
                down_q = _turboquant_weight(down_w, args.bit, args.group_size, seed, args.rotation)

                # 把 top-sensitive neuron 对应的 up/gate 行和 down 列恢复成原始 fp16 权重。
                ...
                # 计算恢复后的输出 y_pred，并只记录 cosine 相关指标。
            ...
            # 一个 preserve fraction 下汇总多个 seed 的 cosine 均值和波动。
        ...
        # 只画 1 - cosine similarity。数值越低，说明输出方向越接近 fp16。
        ...
        ax.set_xlabel("Preserved top-sensitive fraction")
        ax.set_ylabel("1 - cosine similarity")
        ax.set_title("Cosine drift after high-sensitivity fp16 protection")
        ax.grid(True, alpha=0.3)
        fig.suptitle(f"{model_label(model_id)} L{target.layer_idx} E{target.expert_idx}", y=1.02)

        fig_path = out_root / f"L{target.layer_idx}_E{target.expert_idx}_cosine.png"
        _save_fig(fig, fig_path)

        # summary.json 只保存 cosine 指标，避免和旧实验里的 rel-L2/max-abs 混在一起。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dataset", default="wikitext2", choices=["wikitext2", "ptb", "c4"])
    parser.add_argument("--nsamples", type=int, default=64)
    parser.add_argument("--data-seed", type=int, default=42)
    parser.add_argument("--rank-mode", default="turboquant_innerproduct")
    parser.add_argument("--quantmode", default="turboquant", choices=["turboquant", "gptq"])
    parser.add_argument("--sensitivity-source", default="inline", choices=["inline", "cache"])
    ...
    parser.add_argument("--top-layers", type=int, default=1)
    ...
    parser.add_argument("--seeds", nargs="+", type=int, default=[0, 42, 84, 126, 168])
    parser.add_argument("--preserve-fracs", nargs="+", default=["0.0", "0.01", "0.02", "0.05"])
    parser.add_argument("--standby-layer-cpu", action="store_true", help="Accepted for script compatibility; ignored here.")
    return parser


def main() -> None:
    run_cosine_experiment(build_parser().parse_args())
